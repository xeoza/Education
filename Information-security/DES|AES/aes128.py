...
nk = 4  # длина ключа в 32-х битных словах. Для AES, Nk = 4, 6, 8.
...
def sub_bytes(state, inv=False):
    ...
    Сама же таблица получена посредством преобразований уже известного нам поля GF(28)
...
def key_expansion(key):
    ...
    каждый из которых равен по размеру State. Первый раундовый ключ заполняется на основе секретного ключа, который вы придумаете, по формуле
...
def add_round_key(state, key_schedule, round=0):
    ...
    RoundKey — массив такого же размера, как и State, который строится для каждого раунда на основе секретного ключа функцией KeyExpansion()
    """
...
def mul_by_02(num):
    ...
    а затем к результату сдвига применяется операция XOR со значением {1b}. Результат может перескочить за значение {ff}, 
    то есть за границы одного байта. В этом случае нужно вернуть остаток от деления результата на {100}."""
