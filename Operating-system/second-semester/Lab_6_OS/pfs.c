...
static struct super_operations const pfs_super_ops = {
    .put_super = pfs_put_super, //Диструктор суперблока
...
static int pfs_fill_sb(struct super_block *sb, void *data, int silent)
{
    ...
    root = pfs_make_inode(sb, S_IFDIR | 0755); //Создание inode для корневого котолога 
    if (!root)
    {
...
//Функция монтирования ФС
static struct dentry *pfs_mount(struct file_system_type *type, 
                                int flags, char const *dev, void *data)
{
    struct dentry *const entry = mount_bdev(type, flags, dev, data, pfs_fill_sb); // pfs_fill_sb - указатель на функцию для инициализации суперблока
    if (IS_ERR(entry))
        printk(KERN_ERR "pfs mounting failed!\n");
    else
        printk(KERN_INFO "pfs mounted\n");
    return entry; //Структура, описывающая корневой католог ФС
}

//Структура, описывающая фаловую систему 
static struct file_system_type pfs_type = {
    .owner = THIS_MODULE,  //Счетчик ссылок на модуль
    .name = "pfs", //Название ФС (оно используется при монтировании)
    .mount = pfs_mount, //Функция для монтирования ФС
    .kill_sb = kill_block_super, //Функция для размонтирования ФС
