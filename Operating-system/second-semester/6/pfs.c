...
{
    //структура описывающая корневой каталог файловой системы
    struct dentry *const entry = mount_nodev(type, flags, data, pfs_fill_sb); 
    if (IS_ERR(entry))
...
static struct file_system_type r = {
    .owner = THIS_MODULE,
    .name = "pfs",
    .mount = pfs_mount,
