...
void daemonize(const char *cmd)
{
    ...
    // 2. получение максимального возможного номера дискриптора, чтобы закрыть все ненужные 
    if (getrlimit(RLIMIT_NOFILE, &rl) < 0)
        perror("Невозможно получить максимальный номер дискриптора!\n");
    
    // 3. действия, гарантирующие, что процесс не является лидером группы
    // необходимо для вызова setsid()
    if ((pid = fork()) < 0)
        perror("Ошибка функции fork!\n");
    else if (pid != 0) //родительский процесс
        exit(0);
    
    // 4. демон - лидер новой сессии, лидер новой группы
    setsid();
	...
	// невозможность обретения терминала в будущем
    //if ((pid = fork()) < 0)
        //perror("Ошибка функции fork!\n");
    //else if (pid != 0) //родительский процесс
       // exit(0);
